...
public class poker {
    ...
    // Winning cards highlights
    ArrayList<Card> winningPlayerCards = new ArrayList<>();
    ArrayList<Card> winningDealerCards = new ArrayList<>();
    ...
    JPanel gamePanel = new JPanel() {
        ...
        public void paintComponent(Graphics g) {
            ...
            try {   
                ...
                g2.setStroke(new BasicStroke(4)); // Thicker border for highlighting
                ...
                for (int i = 0; i < playerHand.size(); i++) {
                    ...
                    g.drawImage(cardImg, cardX, yOffset, cardWidth, cardHeight, null);
                    ...
                    if (play && winningPlayerCards.contains(card)) {
                        g2.setColor(new Color(5, 181, 49));
                        g2.drawRoundRect(cardX - 2, yOffset - 2, cardWidth + 3, cardHeight + 3, 10, 10);
                    }
                }
            ...
            public void actionPerformed(ActionEvent e) {
                if (drawsRemaining > 0) {
                    ...
                    if (drawsRemaining == 0) {
                        ...
                        resultMessage = "Cards redrawn. Draws remaining " + drawsRemaining;
                    }
                    gamePanel.repaint();
                }
            }
    ...
    public void startGame() {
        ...
        winningPlayerCards.clear();
        winningDealerCards.clear();
    ...
    public void determineWinner() {
        ...
        if (playerHandRank > dealerHandRank) {
            ...
            winningPlayerCards = getWinningCards(playerHand, playerHandRank);
            return;
        } else if (dealerHandRank > playerHandRank) {
            resultMessage = "Dealer's " + dealerHandName + " wins against Your " + playerHandName + ".";
            winningDealerCards = getWinningCards(dealerHand, dealerHandRank);
            return;
        }
    ...
    private ArrayList<Card> getWinningCards(ArrayList<Card> hand, int rank) {
        ArrayList<Card> result = new ArrayList<>();
        
        // Straight Flush, Flush, Straight: all cards contribute
        if (rank == 10 || rank == 6 || rank == 5) { 
        ...
        for (Card card : hand) {
            int count = valueFrequencies.get(card.getValue());
            if (count >= 2) {
                result.add(card);
            }
        }
        ...
        // High Card fallback
        if (result.isEmpty() && !hand.isEmpty()) {
            Card maxCard = Collections.max(hand);
            result.add(maxCard);
        }
    ...
    private int getHandRank(ArrayList<Card> hand) {
        ...
        if (tripleCount == 1 && pairCount == 1) return 7; //Full house
    ...
    public void save() {
        File myFile = new File("Poker_save_Data.frog");
